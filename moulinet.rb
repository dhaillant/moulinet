...
def afficher_eleve(eleve, index=0)
	unless eleve == nil
		...
		texte += " %-16s | %-12s | %-18s | %-8s | %-9s | %-10s | %-11s | %-11s | %-8s" % [eleve[:nom], eleve[:prenom], eleve[:login], eleve[:div], eleve[:divprec], eleve[:date], eleve[:idext],  eleve[:idnat], eleve[:sortie]]
	end
...
def afficher_tous(eleves)
	...
	entete = "     %-16s | %-12s | %-18s | %-8s | %-8s | %-10s | %-11s | %-11s | %-8s" % ["NOM", "Prenom", "Login", "Classe", "Classe -1", "Naissance", "ID EXT", "ID NAT", "Sortie"]
	puts entete
	puts "-" * entete.length
	i = 0
	eleves.each do |e|
...
def afficher_10_premiers(eleves)
	...
	#entete = "     %-12s %-12s %-14s %-8s %-8s %-9s %-11s %-11s %-8s" % ["NOM", "Prenom", "Login", "Classe", "Classe -1", "Naissance", "ID EXT", "ID NAT", "Sortie"]
	entete = "     %-16s | %-12s | %-18s | %-8s | %-9s | %-10s | %-11s | %-11s | %-8s" % ["NOM", "Prenom", "Login", "Classe", "Classe -1", "Naissance", "ID EXT", "ID NAT", "Sortie"]
	puts entete
	puts "-" * entete.length
	i = 0
	10.times do
...
def compter_eleves_sconet_presents liste_sconet
	...
	liste_sconet.each do |eleve_sconet|
		if (eleve_sconet[:sortie] == "" && eleve_sconet[:div] != "")
			presents = presents + 1
		end
	end
